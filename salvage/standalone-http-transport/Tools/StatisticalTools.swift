...
// MARK: - 8. Concordance Analysis (Kendall's W)

public struct ConcordanceAnalysisTool: MCPToolHandler, Sendable {
    public let tool = MCPTool(
        name: "concordance_analysis",
        description: """
        Calculate Kendall's W coefficient of concordance with full significance testing.

        Measures agreement among multiple judges/raters ranking the same set of items.
        Returns W (with and without tie correction), Friedman chi-square, p-value,
        F-statistic, and degrees of freedom.

        Input: Either a full ranking matrix (rankings) OR pre-computed rank sums
        (rank_sums + judges + items).

        Rankings matrix format: Each row is one judge's rankings, each column is one item.
        Example: [[1,2,3,4], [1,3,2,4], [2,1,3,4]] = 3 judges ranking 4 items.
        Use null/0 for missing ratings (Brueckl 2011 pairwise approach).

        Set permutation_test=true to get a permutation-based p-value (useful for small samples).
        ...
        • 0.7-1.0 = Strong to perfect agreement

        References: Kendall & Babington Smith (1939), Legendre (2005), Brueckl (2011)
        """,
        inputSchema: MCPToolInputSchema(
            properties: [
                "rankings": MCPSchemaProperty(
                    type: "array",
                    description: "2D ranking matrix: array of arrays where each inner array is one judge's ranks. Use null or 0 for missing ratings."
                ),
                "rank_sums": MCPSchemaProperty(
                    type: "array",
                    description: "Pre-computed rank sums (alternative to full matrix). Requires judges and items params.",
                    items: MCPSchemaItems(type: "number")
                ),
                "judges": MCPSchemaProperty(
                    type: "number",
                    description: "Number of judges (required when using rank_sums)"
                ),
                "items": MCPSchemaProperty(
                    type: "number",
                    description: "Number of items (required when using rank_sums)"
                ),
                "permutation_test": MCPSchemaProperty(
                    type: "boolean",
                    description: "If true, also run a permutation test for p-value (recommended for small samples)"
                ),
                "permutations": MCPSchemaProperty(
                    type: "number",
                    description: "Number of permutations for the permutation test (default: 10000)"
                ),
                "has_missing": MCPSchemaProperty(
                    type: "boolean",
                    description: "If true, treat 0 values in rankings as missing data and use Brueckl (2011) pairwise approach"
                )
            ],
            required: []
        )
    ...
    public func execute(arguments: [String: AnyCodable]?) async throws -> MCPToolCallResult {
        guard let args = arguments else {
            throw ToolError.invalidArguments("Missing arguments. Provide 'rankings' matrix or 'rank_sums' with 'judges' and 'items'.")
        }

        let hasMissing = args.getBoolOptional("has_missing") ?? false
        let doPermutation = args.getBoolOptional("permutation_test") ?? false
        let permCount = args.getIntOptional("permutations") ?? 10000

        let result: ConcordanceResult<Double>
        var completeRankings: [[Double]]? = nil

        if let rankings = try? args.getDouble2DArray("rankings") {
            guard !rankings.isEmpty else {
                throw ToolError.invalidArguments("Rankings matrix must not be empty")
            }
            guard let firstRow = rankings.first, firstRow.count >= 2 else {
                throw ToolError.invalidArguments("Need at least 2 items to measure concordance")
            }
            let expectedCols = firstRow.count
            for (i, row) in rankings.enumerated() {
                guard row.count == expectedCols else {
                    throw ToolError.invalidArguments("Row \(i) has \(row.count) items, expected \(expectedCols)")
                }
            }
            ...
            if hasMissing {
                ...
                completeRankings = rankings
            }
        } else if let rankSums = try? args.getDoubleArray("rank_sums") {
            guard let judges = args.getIntOptional("judges") else {
                throw ToolError.invalidArguments("'judges' is required when using 'rank_sums'")
            }
            guard let items = args.getIntOptional("items") else {
                throw ToolError.invalidArguments("'items' is required when using 'rank_sums'")
            }
            result = try concordanceAnalysisFromRankSums(
                rankSums: rankSums, judges: judges, items: items)
        } else {
            throw ToolError.invalidArguments(
                "Provide either 'rankings' (2D matrix) or 'rank_sums' with 'judges' and 'items'")
        }

        var permLine = ""
        if doPermutation, let rankings = completeRankings {
            let permResult = try concordancePermutationTest(rankings, permutations: permCount)
            let permSigText = permResult.pValue < 0.05 ? "Significant" : "Not significant"
            permLine = """

            Permutation Test (\(permCount) permutations):
            • Permutation p-value: \(formatNumber(permResult.pValue, decimals: 6))
            • \(permSigText) at α = 0.05
            """
        } else if doPermutation {
            permLine = "\n• Permutation test requires full ranking matrix (not rank_sums or missing data)"
        }

        let wInterp = result.wCorrected > 0.7 ? "Strong" :
                      result.wCorrected > 0.5 ? "Good" :
                      result.wCorrected > 0.3 ? "Moderate" :
                      result.wCorrected > 0.1 ? "Weak" : "Very weak"
        let sigText = result.pValue < 0.001 ? "Highly significant (p < 0.001)" :
                     result.pValue < 0.01 ? "Very significant (p < 0.01)" :
                     result.pValue < 0.05 ? "Significant (p < 0.05)" :
                     result.pValue < 0.10 ? "Marginally significant (p < 0.10)" :
                     "Not significant (p ≥ 0.10)"

        let tieInfo = result.totalTieCorrection > 0
            ? "• Tie correction applied: ΣTⱼ = \(formatNumber(result.totalTieCorrection, decimals: 1))"
            : "• No tied ranks detected"

        let methodNote = hasMissing ? "\n• Method: Brueckl (2011) pairwise Spearman ρ approximation" : ""

        let output = """
        Concordance Analysis (Kendall's W):

        Agreement:
        • Kendall's W: \(formatNumber(result.w, decimals: 6))
        • W (tie-corrected): \(formatNumber(result.wCorrected, decimals: 6))
        • Interpretation: \(wInterp) agreement

        Significance Tests:
        • Chi-square: \(formatNumber(result.chiSquare, decimals: 4)) (df = \(result.degreesOfFreedom))
        • Friedman chi-square: \(formatNumber(result.friedman, decimals: 4))
