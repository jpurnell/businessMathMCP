...
extension Dictionary where Key == String, Value == MCP.Value {
    ...
    /// Get a required 2D array of doubles (array of arrays)
    public func getDouble2DArray(_ key: String) throws -> [[Double]] {
        guard let value = self[key] else {
            throw ValueExtractionError.missingRequiredArgument(key)
        }
        guard let outerArray = value.arrayValue else {
            throw ValueExtractionError.invalidArguments("\(key) must be an array of arrays")
        }

        var result: [[Double]] = []
        for (rowIndex, row) in outerArray.enumerated() {
            guard let innerArray = row.arrayValue else {
                throw ValueExtractionError.invalidArguments("\(key)[\(rowIndex)] must be an array of numbers")
            }
            var rowResult: [Double] = []
            for (colIndex, item) in innerArray.enumerated() {
                if let doubleValue = item.doubleValue {
                    rowResult.append(doubleValue)
                } else if let intValue = item.intValue {
                    rowResult.append(Double(intValue))
                } else {
                    throw ValueExtractionError.invalidArguments("\(key)[\(rowIndex)][\(colIndex)] must be a number")
                }
            }
