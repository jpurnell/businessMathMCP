...
| `String(format: ...)` with C-style specifiers | `%s` expects C string pointer, crashes when given Swift String. Width specifiers like `%-30s` crash. Type errors caught at runtime, not compile time. | String interpolation `"\(value)"`, or `value.formatted()`, or `String.padding(toLength:withPad:startingAt:)`. **See §3.7 for full details.** |
...
public func firstValue() throws -> T {
    guard let first = values.first else {
        throw ProjectError.invalidInput(message: "Cannot get first value of empty collection")
    }
    return first
}
```
...
guard let doubleValue = value as? Double else {
    throw ProjectError.typeMismatch(
        expected: "Double",
...
do {
    ...
    throw ProjectError.operationFailed(underlying: error)
}
...
public func median<T: Real>(_ x: [T]) throws -> T {
    guard !x.isEmpty else {
        throw ProjectError.invalidInput(message: "Cannot compute median of empty array")
    }
    let sorted = x.sorted()
...
guard abs(denominator) > T.ulpOfOne else {
    throw ProjectError.divisionByZero(
...
guard stdDevX > T.ulpOfOne && stdDevY > T.ulpOfOne else {
    throw ProjectError.undefinedStatistic(
...
guard abs(doubleValue) < Double(Int.max) / 1_000_000 else {
    throw ProjectError.numericOverflow(
...
throw ProjectError.convergenceFailure(
...
func compute() throws -> T {
    ...
    throw ProjectError.convergenceFailure(message: "Recursion limit exceeded")
}
```
...
guard let firstRow = matrix.first else {
    throw ProjectError.invalidInput(message: "Matrix cannot be empty")
}
...
guard array.count >= 2 else {
    throw ProjectError.insufficientData(
...
guard arrayA.count == arrayB.count else {
    throw ProjectError.dimensionMismatch(
...
### String Formatting (MANDATORY — Zero Tolerance)

**`String(format:)` is FORBIDDEN in production code, tests, and validation playgrounds.**

This is on the [Forbidden Patterns](#forbidden-patterns-mandatory) list at the
top of this document. It's repeated in detail here because the failure modes
are subtle and recurring.

#### Why it's banned

`String(format:)` is a Swift bridge to NSString's `+stringWithFormat:`, which
in turn uses C's `printf`-family format ABI. That ABI is unsafe at the
language boundary because:

1. **`%s` expects a `const char *` (C string pointer), NOT a Swift `String`.**
   Passing a Swift String causes the runtime to read random bytes after the
   String value, then dereference them as a pointer. **Crashes with a stack
   trace ending in `__CFStringAppendFormatCore` →
   `_CFStringCreateWithFormatAndArgumentsReturningMetadata` → SIGSEGV (exit
   139)**. The Swift compiler accepts the call without warning because
   `String` conforms to `CVarArg`.
2. **C-style width and alignment specifiers like `%-30s`, `%8d`, `%10.3f`
   are not supported** in the same form as C `printf`. They look like they
   should work but produce different output or crash.
3. **All type errors are caught at runtime, not at compile time.** The format
   string is opaque to the type system. The wrong number of arguments, the
   wrong type, the wrong specifier — none of these produce compile errors.
4. **Output is locale-dependent** unless you pass an explicit `Locale`, which
   most callers forget. This means tests can pass on a developer machine and
   fail in CI due to decimal-separator differences.

#### Diagnostic fingerprint

If you see this in a stack trace, you have a `String(format:)` bug:

```
Foundation               $sSS10FoundationE6format6locale9argumentsS2Sh_AA6LocaleVSghSays7CVarArg_pGhtcfc...
CoreFoundation           __CFStringAppendFormatCore + ...
CoreFoundation           _CFStringCreateWithFormatAndArgumentsReturningMetadata + ...
libsystem_platform.dylib _sigtramp + ...
```

Exit code 139 (SIGSEGV) running a Swift script is almost always this.

#### ❌ Forbidden

```swift
// CRASH — %s with Swift String dereferences random memory as a pointer
let line = String(format: "%s passed", testName)

// CRASH — width specifiers
let row = String(format: "%-30s %8d %10.3fs", name, count, time)

// SUBTLY WRONG — locale-dependent output
let pct = String(format: "%.2f%%", value)  // "1,23%" in de_DE, "1.23%" in en_US

// EVEN "SAFE" USES — banned because the next change might add a %s
let pretty = String(format: "%.3f", time)
```

#### ✅ Required alternatives

```swift
// String interpolation — type-safe, locale-neutral, compile-checked
let line = "\(testName) passed"

// Numbers with decimal places — Swift's native FormatStyle
let pct = value.formatted(.percent.precision(.fractionLength(2)))
let pretty = time.formatted(.number.precision(.fractionLength(3)))

// Padding for column alignment — String.padding
...
print("\(opHeader) \(countHeader)")

// For decimal places when FormatStyle isn't available, use a project helper
extension Double {
    /// Round to `decimals` places and return as a String. Locale-neutral.
    func toString(decimals: Int) -> String {
        let multiplier = pow(10.0, Double(decimals))
        let rounded = (self * multiplier).rounded() / multiplier
        return String(rounded)  // Swift's default Double→String is locale-neutral
    }
}

// Then:
let pretty = time.toString(decimals: 3)
```

#### Mechanical enforcement (run before committing)

```bash
# Quality gate: should produce zero matches
grep -rn 'String(format:' --include='*.swift' Sources/ Tests/

# Pre-commit hook one-liner:
git diff --cached --name-only --diff-filter=ACM | grep '\.swift$' | \
    xargs grep -l 'String(format:' && \
    echo "ERROR: String(format:) is forbidden — see coding_rules.md §3.7" && \
    exit 1
```

If quality-gate doesn't yet check this, **add it now**. Any violation should
fail the gate with a clear pointer back to this section.

#### Old examples (kept for context — same rule applies)
```swift
...
> **📖 Full Reference: [DocC Guidelines](docc_guidelines.md)**
>
...
### Custom Error Types
- Create dedicated error enums within the 'ProjectError' superdomain
...
### Fail-Silent Principle (Prefer No-Answer Over Wrong-Answer)

> *Inspired by NASA's Artemis II fault-tolerant architecture, where a faulty computer
> "fails silent rather than transmitting the wrong answer."*

Functions must **never return plausible-but-wrong results** when they detect a degraded condition. They must either throw, return nil/Optional, or annotate the result with a degradation flag. A wrong answer that *looks* correct is more dangerous than an explicit failure.

#### Violation Patterns to Watch For

1. **Silent fallback paths** — A function switches from one computation method to another (e.g., GPU to CPU, fast-path to slow-path) without notifying the caller. If the fallback changes precision, coverage, or semantics, the caller must know.

2. **Returning "best-so-far" without degradation flag** — Iterative algorithms (optimizers, solvers) that encounter NaN/Inf mid-computation and return their last-known-good result *as if* it were a converged solution. The result must be marked as non-converged or degraded.

3. **Compilation/parsing failures producing default outputs** — A subsystem that silently stores empty or default values when initialization fails, rather than throwing. Downstream code then operates on the empty state without realizing.

#### Required Pattern

When a function detects a degraded condition, choose one of:

```swift
// Option A: Throw (preferred when caller must handle)
guard gradient.allSatisfy({ $0.isFinite }) else {
...
// Option B: Annotated result (when degradation is recoverable/informational)
struct SimulationResults {
    let statistics: SimulationStatistics
    let executionNotes: [String]  // Records fallback paths taken
    var isDegraded: Bool { !executionNotes.isEmpty }
}

// Option C: Return nil (when Optional is the natural return type)
func solve(equation: Expression) -> Solution? {
    guard equation.isWellFormed else { return nil }
    // ...
}
```
...
**Never** do this:
```swift
// ❌ Silent fallback — caller has no idea GPU failed
func run() -> SimulationResults {
    do {
        return try runGPU()
    } catch {
        print("GPU failed, falling back to CPU")  // Only goes to console!
        return runCPU()  // Different precision, caller unaware
    }
}
```

### Stateful Recovery (Re-Initialization from Known-Good State)

Stateful components — simulation engines, optimizers, caches, connection pools — must support re-initialization from a known-good state without requiring full deallocation and reconstruction. This enables recovery from transient failures without restarting entire pipelines.

```swift
/// A component that can be reset to a known-good initial state.
protocol Resettable {
    /// Restores the component to its initial configuration.
    /// After calling reset(), the component must behave identically
    /// to a freshly constructed instance with the same configuration.
    func reset() throws
}
```

**When to apply:**
- Long-running pipelines where one stage may corrupt internal state
- Components that accumulate state across multiple invocations (buffers, caches)
- Systems where a mid-pipeline failure should allow retry without full restart
...
> **📖 Full References:**
> - **[Test-Driven Development Directive](test_driven_development.md)** — Comprehensive TDD standard
...
- Testing imports: `Testing`, `@testable import [PROJECT_NAME]`
...
- **Minimum Required**: Swift 6.0
- **Tools Version**: `swift-tools-version: 6.0` in Package.swift

Swift 6 is the baseline. Strict concurrency is enforced by the language mode, not opt-in flags.
...
1. **Swift Tools Version**: Must be `6.0` or later
   ```swift
   // ✅ CORRECT
   // swift-tools-version: 6.0

   // ❌ WRONG - Legacy version, missing native concurrency enforcement
   // swift-tools-version: 5.9
   ```

2. **Strict Concurrency is the default**: Swift 6 language mode enforces data-race safety at compile time. No `swiftSettings` flags needed.
...
|--------------|--------|-------|
| 6.0.x | ✅ Supported | Minimum required version |
| 6.1+ | ✅ Supported | Recommended for latest fixes |
| < 6.0 | ❌ Not Supported | Missing native concurrency enforcement |
...
```bash
swift build   # Swift 6 language mode enforces strict concurrency by default
swift test
```

### Strict Concurrency Rules

1. **Always enforced**: Swift 6 language mode enables strict concurrency natively
...
### Importing Non-Sendable Framework Types

When importing Apple frameworks whose types predate Swift concurrency (Metal, CoreGraphics, etc.), use `@preconcurrency import` rather than marking the containing type `@unchecked Sendable`:

```swift
// ✅ Correct: tells compiler these types predate Sendable
#if canImport(Metal)
@preconcurrency import Metal
#endif

public struct GPUBackend: Sendable {
    private let device: MTLDevice        // No warning
    private let commandQueue: MTLCommandQueue  // No warning
}
```

```swift
// ❌ Wrong: hides the real issue behind unchecked
import Metal

public struct GPUBackend: @unchecked Sendable {
    private let device: MTLDevice  // Unsafe — compiler can't verify
}
```

**Why:** `@preconcurrency import` is scoped to the import and preserves the compiler's ability to check your own types. `@unchecked Sendable` on the struct silences ALL Sendable checking for the type, including future properties you add.

### Generic Default Parameters and Type Inference

When a generic type `T` has default values on `init` parameters, Swift 6.1+ warns that defaults should not participate in type inference when `T` is already inferrable. Use the **constrained extension pattern**:

```swift
// ✅ Correct: defaults in constrained extension
public struct Optimizer<T: FloatingPoint> {
    public init(tolerance: T, maxIterations: Int) { ... }
}

extension Optimizer where T == Double {
    public init(tolerance: Double = 0.0001, maxIterations: Int = 100) {
        self.init(tolerance: tolerance, maxIterations: maxIterations)
    }
}
```

```swift
// ❌ Will become an error in future Swift: defaults on generic T
public struct Optimizer<T: FloatingPoint> {
    public init(tolerance: T = 0.0001, maxIterations: Int = 100) { ... }
}
```
...
- Use `@unchecked Sendable` on structs to suppress framework import warnings (use `@preconcurrency import` instead)
- Create `Range` from unvalidated values that could produce `lowerBound > upperBound` (e.g., `1..<array.count` when count could be 0)
...
- Guard Range construction: `guard collection.count > 1 else { return }` before `1..<collection.count`
- Use `@preconcurrency import` for Apple framework modules with non-Sendable types

### `@unchecked Sendable` Audit Checklist

Every `@unchecked Sendable` type must satisfy ALL of the following:

1. **Lock protection**: Every mutable stored property is accessed only while holding an `NSLock`, `os_unfair_lock`, or equivalent
2. **Documented guarantees**: Doc comment explains the synchronization strategy
3. **TSan verified**: Has been tested with Thread Sanitizer without warnings
4. **No unprotected accessors**: Computed properties that read mutable state also hold the lock

```swift
// ✅ Correct: all mutable state protected
...
```swift
// ❌ Wrong: @unchecked Sendable with unprotected mutation
public final class ConfigStore: @unchecked Sendable {
    var values: [String: Double] = [:]  // No lock — data race
}
```

---

## 10. Generic Expression Type-Check Performance (MANDATORY)

### The Problem

The Swift type-checker's performance varies significantly across compiler versions. Newer compilers handle complex generic expressions in milliseconds, but older environments (Swift 6.0.3, Xcode 16.x) timeout on the same expressions. This causes CI failures on code that compiles locally.

### Enforcement: Compiler Threshold Flag

CI workflows and the quality-gate `BuildChecker` enforce a 500ms per-expression type-check limit:

```bash
swift build -Xswiftc -Xfrontend -Xswiftc -solver-expression-time-threshold=500
```

This is the authoritative check — if it passes, CI will pass. Run it locally before pushing.

### When Expressions Timeout: Remediation Strategies

If the threshold flag catches a slow expression, use one of these approaches:

**Option A: Add explicit type annotations** — eliminates overload search entirely:
```swift
// Slow: compiler must infer types through 6 overloaded operators
score += T(-1) / T(2) * trPdV + T(1) / T(2) * rPdVPr

// Fast: explicit `: T` annotations guide the solver
let term1: T = T(-1) / T(2) * trPdV
let term2: T = T(1) / T(2) * rPdVPr
score += term1 + term2
```

**Option B: Break into intermediate bindings** — reduces operator chain length:
```swift
// Slow: deep nested expression
scoreE += T(-1) / T(2) * ((nig - T(1)) / sigmaE2 + T(1) / ai
    - ssWithin / (sigmaE2 * sigmaE2) - sumResid * sumResid / ai2)
...
let scoreBody: T = termA + termB - termC - termD
scoreE += T(-1) / T(2) * scoreBody
```

### Why This Happens

The Swift constraint solver explores type combinations exponentially with the number of overloaded operators. With generic types (`T: Real`), each `+`, `*`, `/` has multiple candidate conformances. Explicit type annotations or shorter chains reduce the search space.

As compilers improve, fewer expressions will hit the threshold. The flag-based enforcement automatically adapts — no code changes needed when the solver gets faster.

---

## 11. GPU Acceleration Architecture (MANDATORY)
...
If your project includes GPU acceleration (e.g., for genetic algorithms), follow the same patterns:
...
Projects commonly use random numbers for Monte Carlo simulations, stochastic optimization (genetic algorithms, simulated annealing), and statistical sampling.
...
## 16. Memory Management (MANDATORY)
...
For every commit:
- [ ] **Swift 6 Compliance (MANDATORY)**: `swift build` passes with **zero concurrency errors**
- [ ] **All Tests Pass**: `swift test` completes successfully
- [ ] Code compiles on Swift 6.0+ (minimum version)
...
- **Security audit** — force unwraps, division by zero, overflow, randomness. Run via `quality-gate --check safety,fp-safety,stochastic-determinism`; record findings in `project/summaries/`.
- **Memory audit** — unbounded collections, GPU resources, async tasks, caches. Run via `quality-gate --check memory-lifecycle`; record findings in `project/summaries/`.
