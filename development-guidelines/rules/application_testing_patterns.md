...
[test_driven_development.md](test_driven_development.md) defines the unit-level TDD contract. This document extends that foundation with **application-level testing patterns** — integration tests, end-to-end workflows, plugin pipelines, performance benchmarks, and coverage targets.
...
```
     ...
     /------------\
    /              \  View Model + State Matrix Tests (15%)
   /----------------\
  /                  \  Unit Tests — Logic + View Models (45%)
 /____________________\
```

| Layer | Share | Purpose |
|-------|-------|---------|
| **Unit** | 45% | Pure functions, deterministic calculations, view model state logic |
| **View Model** | 15% | State-matrix coverage, view model transitions, computed properties ([ui_testing.md](ui_testing.md)) |
| **Integration** | 25% | Multi-component interactions, pipelines |
| **E2E** | 10% | Full user workflows, UI through backend |
| **Manual** | 5% | Exploratory testing, animation feel, gesture responsiveness |
...
Store checklists in `project/checklists/` or alongside the feature's roadmap documentation.
...
| UI view models | 95%+ | 100% state-matrix coverage |
...
- [UI Testing](ui_testing.md) — state-matrix coverage and view model testing
