...
> | **[Release Checklist](../rules/release_checklist.md)** | Before tagging a release | Verification only |
>
...
**Key Documents:**
- [Design Proposal Phase](../rules/design_proposal.md) — Step 0 requirements
- [Test-Driven Development](../rules/test_driven_development.md) — Steps 1-3 requirements
...
> **See [Design Proposal Phase](../rules/design_proposal.md) for full template.**
...
- [ ] **State matrix** documented for UI views (see [UI Testing](../rules/ui_testing.md))
...
- [ ] **State-matrix tests** (UI features) — View model tests covering all states, transitions, and computed properties per [ui_testing.md](../rules/ui_testing.md)
- [ ] **Test quality evaluation** — Run `/evaluate-tests` and score >= 75 on all dimensions
...
```bash
# Evaluate test quality BEFORE moving to GREEN
# All 5 dimensions must score >= 75 (Good)
/evaluate-tests Tests/NewFeatureTests/
```
...
- [ ] DocC catalog created — **required for QualityChecker modules:**
  - [ ] `Sources/<Module>/<Module>.docc/<Module>.md` — root doc with rules table
  - [ ] `Sources/<Module>/<Module>.docc/<Module>Guide.md` — walkthrough with code examples
...
| `test-quality` | No test anti-patterns (exact `==` on Double, `try!`, unseeded RNG, weak assertions) |
...
### Safety Audit
> **Full list of forbidden patterns:** See [Coding Rules](../rules/coding_rules.md#forbidden-patterns-mandatory)
