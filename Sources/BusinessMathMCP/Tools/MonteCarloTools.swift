        ...
        inputSchema: MCPToolInputSchema(
            properties: [
                "inputs": MCPSchemaProperty(
                    ...
                    • distribution (string): Distribution type (see full list below)
                    • parameters (object): Distribution-specific parameters

                    Supported distributions (15 total):
                    • normal: {mean, stdDev} - Bell curve, continuous symmetric
                    • uniform: {min, max} - Equal probability across range
                    • triangular: {min, max, mode} - Three-point estimate
                    • lognormal: {mean, stdDev} - For multiplicative processes (stock prices)
                    • exponential: {rate} - Time between events
                    • beta: {alpha, beta} - Bounded (0-1), flexible shape
                    • gamma: {shape, scale} - Waiting times, durations
                    • weibull: {shape, scale} - Reliability, failure analysis
                    • chisquared: {degreesOfFreedom} - Goodness-of-fit tests
                    • f: {df1, df2} - ANOVA, variance comparison
                    • t: {degreesOfFreedom} - Small sample inference
                    • pareto: {scale, shape} - 80/20 rule, wealth distribution
                    • logistic: {mean, stdDev} - Growth models, S-curves
                    • geometric: {p} - Trials until first success
                    • rayleigh: {mean} - Magnitude modeling (wind, waves)
    ...
    public func execute(arguments: [String: AnyCodable]?) async throws -> MCPToolCallResult {
        ...
        for (index, inputAnyCodable) in inputsAnyCodable.enumerated() {
            ...
            case "exponential":
                ...
                simInput = SimulationInput(name: name, distribution: DistributionExponential(rate))

            case "beta":
                guard let alpha = params["alpha"], let beta = params["beta"] else {
                    throw ToolError.invalidArguments("Beta distribution requires 'alpha' and 'beta'")
                }
                simInput = SimulationInput(name: name, distribution: DistributionBeta(alpha: alpha, beta: beta))
            ...
            case "weibull":
                ...
                simInput = SimulationInput(name: name, distribution: DistributionWeibull(shape: shape, scale: scale))
            ...
            case "pareto":
                ...
                simInput = SimulationInput(name: name, distribution: DistributionPareto(scale: scale, shape: shape))

            case "logistic":
                guard let mean = params["mean"], let stdDev = params["stdDev"] else {
                    throw ToolError.invalidArguments("Logistic distribution requires 'mean' and 'stdDev'")
                }
                simInput = SimulationInput(name: name, distribution: DistributionLogistic(mean, stdDev))

            case "geometric":
                guard let p = params["p"] else {
                    throw ToolError.invalidArguments("Geometric distribution requires 'p' (probability)")
                }
                simInput = SimulationInput(name: name, distribution: DistributionGeometric(p))
