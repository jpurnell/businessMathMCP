...
struct BusinessMathMCPServerMain {
    static func main() async throws {
        ...
        // Heuristic Optimization Tools (2 tools) - PSO, GA
        for handler in getHeuristicOptimizationTools() {
            try await toolRegistry.register(handler.toToolDefinition())
        }

        // Metaheuristic Optimization Tools (2 tools) - SA, DE
        for handler in getMetaheuristicOptimizationTools() {
            try await toolRegistry.register(handler.toToolDefinition())
        }

        // Advanced Simulation Tools (2 tools) - Correlated MC, GPU MC
        // TODO: Re-enable when BusinessMath main branch API stabilizes
        // for handler in getAdvancedSimulationTools() {
        //     try await toolRegistry.register(handler.toToolDefinition())
        // }

        // Financial Statement Tools (4 tools) - Income Statement, Balance Sheet, Cash Flow, Validation
        for handler in getFinancialStatementTools() {
            try await toolRegistry.register(handler.toToolDefinition())
        }

        // Operational Metrics Tools (2 tools) - SaaS metrics, E-commerce metrics
        for handler in getOperationalMetricsTools() {
            try await toolRegistry.register(handler.toToolDefinition())
        }

        // Capital Structure Tools (2 tools) - WACC, Cost of Equity
        for handler in getCapitalStructureTools() {
            try await toolRegistry.register(handler.toToolDefinition())
        }

        // Enhanced Covenant Tools (1 tool) - Comprehensive covenant monitoring
        for handler in getEnhancedCovenantTools() {
            try await toolRegistry.register(handler.toToolDefinition())
        }

        // Multi-Period Analysis Tools (1 tool) - Financial trend analysis
        for handler in getMultiPeriodAnalysisTools() {
            try await toolRegistry.register(handler.toToolDefinition())
        }

        // Advanced Financial Modeling Tools (1 tool) - Scenario analysis
        for handler in getAdvancedFinancialModelingTools() {
            try await toolRegistry.register(handler.toToolDefinition())
        }
